...
- Signposting **requires** machine interactions with scholarly platforms. Some measures taken by repositories to defend against malicious or badly-configured "bots" may also (perhaps inadvertently) interfere with (e.g. require CAPTCHA tests) or even prevent access from benign processes attempting to use Signposting for legitimate reasons. Care should be taken to allow unimpeded machine interaction from legitimate sources with repositories and other scholarly platforms.
